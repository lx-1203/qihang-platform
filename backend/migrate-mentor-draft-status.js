...
async function migrate() {
  ...
  try {
    ...
    console.log('[migrate] 检查 mentor_profiles 表 verify_status ENUM...');

    const [columns] = await connection.query(
      `SELECT COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'mentor_profiles' AND COLUMN_NAME = 'verify_status'`,
      [DB_NAME]
    );

    if (columns.length === 0) {
      console.log('[migrate] mentor_profiles 表或 verify_status 列不存在，跳过迁移');
      return;
    }
    ...
    await connection.query(
      `ALTER TABLE mentor_profiles MODIFY COLUMN verify_status ENUM('draft','pending','approved','rejected') NOT NULL DEFAULT 'draft' COMMENT '审核状态'`
    );
