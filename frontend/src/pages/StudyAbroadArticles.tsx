      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4 mb-8">
          ...
          <div className="flex items-center gap-2">
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value as typeof sortBy)} className="text-[13px] text-[#4b5563] bg-white border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-primary-500 cursor-pointer">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg transition-all overflow-hidden">
            <div className="flex flex-col md:flex-row">
              ...
              <div className="p-6 md:p-8 flex flex-col justify-center">
                ...
                <h2 className="text-[22px] md:text-[26px] font-bold text-[#111827] mb-3 group-hover:text-primary-500 transition-colors leading-tight">{FEATURED_ARTICLE.title}</h2>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-8">
          <div className="relative mb-4">
            ...
            <input type="text" placeholder="搜索文章标题、内容或标签..." value={searchKeyword} onChange={(e) => setSearchKeyword(e.target.value)}
              className="w-full pl-11 pr-4 py-3 bg-[#f9fafb] border border-gray-200 rounded-xl text-[14px] text-[#111827] focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent placeholder-[#9ca3af] transition-all" />
          </div>
          <div className="flex flex-wrap gap-2">
            {CATEGORIES.map((cat) => (
              <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-lg text-[13px] font-medium transition-all ${selectedCategory === cat ? 'bg-primary-500 text-white shadow-sm' : 'bg-[#f3f4f6] text-[#4b5563] hover:bg-gray-200'}`}>
                {cat}
              </button>
            ))}
          </div>
          ...
          <div className="flex-grow">
                    ...
                    <div className="flex flex-col sm:flex-row">
                      ...
                      <div className="p-5 flex flex-col justify-between flex-grow">
                        <div>
                          ...
                          <h3 className="text-[17px] font-bold text-[#111827] mb-2 line-clamp-2 group-hover:text-primary-500 transition-colors leading-snug">{article.title}</h3>
            ...
            {filtered.length > 0 && (
              <div className="flex justify-center mt-10">
                <div className="flex items-center gap-2">
                  {[1, 2, 3].map((page) => (
                    <button key={page} className={`w-10 h-10 rounded-xl text-[14px] font-medium transition-colors ${page === 1 ? 'bg-primary-500 text-white shadow-sm' : 'bg-white text-[#6b7280] border border-gray-200 hover:border-primary-500 hover:text-primary-500'}`}>
                      {page}
                    </button>
                  ))}
                  <span className="text-[#9ca3af] px-2">...</span>
                  <button className="w-10 h-10 rounded-xl bg-white text-[#6b7280] border border-gray-200 hover:border-primary-500 hover:text-primary-500 text-[14px] font-medium transition-colors">6</button>
                </div>
              </div>
            )}
          </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {HOT_TOPICS.map((topic, idx) => (
                  <button key={idx} className="bg-[#f3f4f6] hover:bg-primary-50 hover:text-primary-500 text-[#4b5563] text-[12px] font-medium px-3 py-1.5 rounded-lg transition-colors">
                    {topic.label}
                ...
                {[...ARTICLES].sort((a, b) => b.views - a.views).slice(0, 5).map((article, idx) => (
                  <Link key={article.id} to={`/study-abroad/articles/${article.id}`} className="flex items-start gap-3 group">
                    <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-[12px] font-bold shrink-0 ${
                      idx < 3 ? 'bg-primary-500 text-white' : 'bg-gray-100 text-[#6b7280]'
                    }`}>{idx + 1}</span>
                    <div className="flex-grow min-w-0">
                      <h4 className="text-[13px] font-medium text-[#374151] line-clamp-2 group-hover:text-primary-500 transition-colors leading-snug">{article.title}</h4>
