...
export default function CompanyResumePool() {
  ...
  async function fetchResumes() {
    try {
      ...
      const raw = res.data?.data?.resumes;
  ...
  function changeStatus(id: number, newStatus: ResumeStatus) {
    ...
    void applyStatusChange(id, newStatus);
  }

  async function applyStatusChange(id: number, newStatus: ResumeStatus) {
    const previousStatus = resumes.find(r => r.id === id)?.status;
    setResumes(prev => prev.map(r => r.id === id ? { ...r, status: newStatus } : r));

    try {
      await http.put(`/company/resumes/${id}/status`, { status: newStatus });
      showToast({ type: 'success', title: '状态已更新' });
    } catch {
      if (previousStatus) {
        setResumes(prev => prev.map(r => r.id === id ? { ...r, status: previousStatus } : r));
      }
      showToast({ type: 'error', title: '状态更新失败', message: '请稍后重试' });
    }
  }
  ...
  async function handleConfirmReject() {
    ...
    try {
      ...
      await applyStatusChange(rejectTarget.id, 'rejected');
    } finally {
  ...
  function onDragEnd(result: DropResult) {
    ...
    const resumeId = parseInt(draggableId, 10);
    const newStatus = destination.droppableId as ResumeStatus;

    if (newStatus === 'rejected') {
      const resume = resumes.find(r => r.id === resumeId);
      setRejectTarget({ id: resumeId, name: resume?.studentName || '' });
      return;
    }

    void applyStatusChange(resumeId, newStatus);
  }
