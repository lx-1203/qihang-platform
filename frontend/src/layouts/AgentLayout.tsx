...
export default function AgentLayout() {
  ...
  const sidebarContent = (
    <>
      ...
      <nav className="flex-1 py-4 px-4 space-y-1 overflow-y-auto" role="navigation" aria-label="客服导航">
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {sidebarOpen && (
        <>
          ...
          <aside className="fixed inset-y-0 left-0 w-64 bg-slate-900 z-40 flex flex-col text-slate-300 md:hidden">
            {sidebarContent}
          </aside>
        </>
      )}
        ...
        <header className="h-16 bg-white border-b border-gray-200 sticky top-0 z-10 flex items-center justify-between px-4 md:px-8 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg md:text-xl font-bold text-gray-800">
              {SIDEBAR_NAV.find(item => location.pathname.includes(item.href))?.name || '客服中心'}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 pl-4 border-l border-gray-200">
              ...
              <div className="hidden sm:flex flex-col">
                <span className="text-sm font-bold text-gray-900 leading-tight">{user?.name || '客服'}</span>
